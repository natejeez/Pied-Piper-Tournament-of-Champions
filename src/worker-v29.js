  ...
  async fetch(request, env, ctx) {
    ...
    if (!url.pathname.startsWith('/api/') && (url.pathname === '/' || url.pathname === '/index.html') && response.ok && (response.headers.get('content-type') || '').includes('text/html')) {
      const transformed = new HTMLRewriter().on('head', {
        element(el) {
          el.append('<style>html.hmpp-boot-pending body{visibility:hidden!important}</style><script>document.documentElement.classList.add("hmpp-boot-pending")</script><link rel="stylesheet" href="/ui-v29.css?v=2.9.2"><script src="/ui-v29.js?v=2.9.2" defer></script><script src="/dev-reset-v29.js?v=2.9.2" defer></script>', { html:true });
        }
      }).transform(response);
      const headers = new Headers(transformed.headers);
      headers.set('x-hmpp-build', 'v2.9.2');
